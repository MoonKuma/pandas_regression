...
def test_all_hypothesis(model_method=model_set_1):
    ...
    _save_result_half_path(save_path, 'FINAL', final_result_test, final_result_train)
...
# save result dict
def _save_result_half_path(save_path, hypothesis, result_test, result_train):
    save_name_test = hypothesis + '_test.txt'
    save_name_train = hypothesis + '_train.txt'
    with open(os.path.join(save_path, save_name_test), 'w') as file_test:
        j = json.dumps(result_test)
        file_test.write(j)
    with open(os.path.join(save_path, save_name_train), 'w') as file_train:
